// Package cli assembles the lyrics command tree from the lyrics
...
// NewApp assembles the kit application from the lyrics domain. The
...
//
// To add a command, declare it in lyrics/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := lyrics.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(lyrics.Domain{}).Register(app)
